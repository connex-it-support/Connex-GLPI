...
class Html {
   ...
   **/
   static function includeHeader($title = '', $sector = 'none', $item = 'none', $option = '') {
      ...
      echo "<head><title>Connex - ".$title."</title>";
      ...
      echo "<link rel='shortcut icon' type='images/x-icon' href='".
             $CFG_GLPI["root_doc"]."/pics/favicon2.ico' >\n";
   ...
   **/
   static function getMenuInfos() {
      ...
      $menu['helpdesk']['title']     = __('Help Desk ');
   ...
   **/
   static function header($title, $url = '', $sector = "none", $item = "none", $option = "") {
      ...
      echo "<div id='c_logo'>";
      echo Html::link('', $CFG_GLPI["root_doc"]."/front/ticket.php",
                      ['accesskey' => '1',
                            'title'     => __('')]);
      echo "</div>";
   ...
   **/
   static function helpHeader($title, $url = '') {
      ...
      echo "<div id='c_logo'>";
      echo "<a href='".$CFG_GLPI["root_doc"]."/front/ticket.php' accesskey='1' title=\"".
   ...
   **/
   static function helpFooter() {
      ...
      echo "<div id=\"footer\" class=\"page\">";
      echo  self::getCopyrightMessage();
      echo "</div>";
   ...
   **/
   ...
   static function displayMenuAll($menu = []) {
      ...
      echo "<a class='primary-fg' href='http://ithelpdesk.connexservice.ca/plugins/formcreator/front/formdisplay.php?id=1' title=\"Home\" class='itemP'>Create a Ticket</a>";
      echo "</dt>";
...
//                               return {results: data.results};
   ...
   **/
   ...
   static function getCopyrightMessage() {
      $message = "<a href=\"http://www.connexservice.ca/\" title=\"Powered By Connex\" class=\"copyright\">";
      $message .= "Connex Telecommunications" .
         " Copyright (C) " . GLPI_YEAR . " ".
         "".
         "</a>";
     return $message;
   }
   ...
   private static function displayMainMenu($full, $options = []) {
      ...
      if ($full === true) {
         ...
         if (Session::haveRight("ticket", CREATE)) {
            $menu['create_ticket'] = [
               'default'   => '/plugins/formcreator/front/formdisplay.php?id=1',
               'title'     => __s('New Hire Form'),
               'content'   => [true]
            ];
         }
         ...
         if (Session::haveRight("ticket", CREATE)) {
            $menu['Statistics'] = [
               'default'   => '/front/stat.php',
               'title'     => __s('Statistics'),
               'content'   => [true]
            ];
         }
      ...
      if ($full === false) {
         // Display Home menu
         echo "<li id='menu1'>";
         echo "<a href='".$CFG_GLPI["root_doc"]."/plugins/formcreator/front/formdisplay.php?id=2' title=\"".
               __s('Home')."\" class='itemP'>".__('Create a Ticket')."</a>";
         echo "</li>";
      }
      ...
      // Display item
     // $mainurl = ($full === true) ? 'central' : 'ticket';
    ...
    //         __('')."</a></li>";
      ...
      // check user id : header used for display messages when session logout
      if (Session::getLoginUserID()) {
         self::showProfileSelecter($CFG_GLPI["root_doc"]."/front/ticket.php");
      }
      echo "</ul>";
      echo "</div>";
   }  
   /**
    * Allow access to statistic pages if user is a manager
	*
	* Manager needs to be on simple interface so cannot be given access to statistics. 
	* This function makes the stat page check if the user's profile is a manager
	*
	* @params none
	* @return none
   **/
   static function manager_stats_header() {
		...
		//if user is a manager, give access to statistics page. Otherwise, check the profile's rights.
		if ($_SESSION['glpiactiveprofile']['id'] !=9 && 
			$_SESSION['glpiactiveprofile']['name'] != "Managers"){
    ...
    * Add chart graphics
	*
	* Simplified header does not load these which manager uses when accessing stats pages. This will load them.
    *
	* @param none
	* @return none
	*
   **/
	static function add_chart_graphics() {
		//load chart css
		echo Html::css('lib/chartist-js-0.10.1/chartist.css');
		echo Html::css('css/chartists-glpi.css');
		echo Html::css('lib/chartist-plugin-tooltip-0.0.17/chartist-plugin-tooltip.css');
