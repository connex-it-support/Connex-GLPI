<?php
/*
 * @version $Id: client.form.php 144 2015-09-06 18:05:31Z yllen $
 -------------------------------------------------------------------------
  LICENSE
...
if (isset($_POST["add"]) && !empty($_POST["name"])) {
   $client->check(-1, 'w', $_POST);
   $client->add($_POST);
   Html::back();

} else if (isset($_POST["delete"])) {
   $client->check($_POST['id'], 'd');
   $client->delete($_POST);
   $client->redirectToList();

} else if (isset($_POST["update"])) {
   $client->check($_POST['id'], 'w');
   $client->update($_POST);
   Html::back();
}

Html::header($client->getTypeName(2), $_SERVER["PHP_SELF"], 'plugins', 'dumpentity', 'client');

$client->showForm($_GET["id"]);
